...
public:
    ...
    std::pair<key_value*, size_t> RunBatchGet(size_t batch_size, GetKeyF get_key) {
        ...
        time_nested("taskgen", [&]() {
            ...
            time_nested("push_task", [&]() {
                batch->push_task_from_array_by_isort<false>(
                    batch_size, [&](size_t i) { return (L3_get_task){.key = get_key(i)}; },
        ...
        parfor_wrap(0, batch_size, [&](size_t i) {
            ...
            if (reply->valid == 1) {
                kv_output[i] = (key_value){.key = get_key(i), .value = reply->value};
            } else {
                kv_output[i] = (key_value){.key = INT64_MIN, .value = INT64_MIN};
            }
        });
        time_end("get_result");
        io->reset();
    ...
    std::pair<key_value*, size_t> RunBatchPredecessor(size_t batch_size, GetKeyF get_key) {
        ...
        time_nested("taskgen", [&]() {
            ...
            time_nested("push_task", [&]() {
                batch->push_task_from_array_by_isort<false>(
                    n, [&](size_t i) { return (L3_search_task){.key = get_key(i)}; },
                    make_slice(target), make_slice(location));
            });
            io->finish_task_batch();
        });
