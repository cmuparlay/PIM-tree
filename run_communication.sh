...
do
    ...
    do
        ...
        do
            ...
            result_str="results/upmem_1/result_micro_communication_${top_level_threads}_${alpha}_${type}.txt"

            ls ${init_file_str}
            ls ${op_file_name}
            ls ${result_str}
            echo "numactl --cpunodebind=0 --membind=0 ./build/pim_tree_host_upmem -f ${init_file_str} ${op_file_name} -l 500000000 100000000 --top_level_threads ${top_level_threads} -c -t -d > ${result_str}"
            numactl --cpunodebind=0 --membind=0 ./build/pim_tree_host_upmem -f ${init_file_str} ${op_file_name} -l 500000000 100000000 --top_level_threads ${top_level_threads} -c -t -d > ${result_str}
        done
    done
done
