...
do
    ...
    do
        ...
        do
            ...
            result_str="results/direct_2/result_micro_${top_level_threads}_${alpha}_${type}.txt"

            ls ${init_file_str}
            ls ${op_file_name}
            echo "numactl --interleave=all ./build/pim_tree_host -f ${init_file_str} ${op_file_name} -l 500000000 100000000 --top_level_threads ${top_level_threads} -c -t -d >> ${result_str}"
            numactl --interleave=all ./build/pim_tree_host -f ${init_file_str} ${op_file_name} -l 500000000 100000000 --top_level_threads ${top_level_threads} -c -t -d >> ${result_str}
        done
    done
done
...
#             echo ${op_file_name}
#             echo "numactl --interleave=all ./build/pim_tree_host -f ${init_file_str} ${op_file_name} -l 500000000 100000000 --top_level_threads ${top_level_threads} -c -t -d > ${result_str}"
#             numactl --interleave=all ./build/pim_tree_host -f ${init_file_str} ${op_file_name} --top_level_threads ${top_level_threads} -c -t -d > ${result_str}
#         done
#     done
# done
