...
HOST_LIB_FLAGS := -isystem pim_base/argparse/include -isystem pim_base/parlaylib/include ${INCLUDE_UPMEM_SRC_LIBS}  -Ipim_base/include/host -Ipim_base/pim_interface -Ipim_base/timer_tree/include
HOST_FLAGS := ${COMMON_FLAGS} -std=c++17 -lpthread -O0 -I${HOST_DIR} ${HOST_LIB_FLAGS} `dpu-pkg-config --cflags --libs dpu` -march=native -DNR_TASKLETS=${NR_TASKLETS} -DNR_DPUS=${NR_DPUS}
